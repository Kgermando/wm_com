...
class BonConsommationPDFA6 extends GetxController {
  generatePdf(List<CartModel> cartListItem, String monnaie) async {
    await Printing.layoutPdf(
        format: PdfPageFormat.a6,
        onLayout: (format) => _generatePdf(cartListItem, monnaie));
  }

  Future<Uint8List> _generatePdf(List<CartModel> cartListItem, monnaie) async {
    final pdf = Document();
  ...
  static Widget buildInvoiceInfo(
      List<CartModel> cartListItem, UserModel user, String monnaie) {
    // final titles = <String>[
    //   'Entreprise:',
    //   'N° Facture:',
    //   'Date:',
  ...
  static Widget buildInvoice(List<CartModel> cartListItem, String monnaie) {
  ...
  static Widget buildTotal(List<CartModel> cartListItem, String monnaie) {
    // var tva = 0.0;
    double sumCart = 0;

    for (var item in cartListItem) {
      // TVA
